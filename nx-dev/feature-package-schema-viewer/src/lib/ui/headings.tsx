...
export const Heading1 = ({ title }: { title: string }) => (
  <h1
    id={slugify(title)}
    className="group mb-5 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-slate-100"
  >
    <span>{title}</span>
...
export const Heading3 = ({ title }: { title: string }) => (
  <h3
    id={slugify(title)}
    className="group text-xl font-semibold tracking-tight text-gray-700 dark:text-slate-300"
  >
    <span>{title}</span>
    <a aria-hidden="true" tabIndex={-1} href={'#' + slugify(title)}>
      <LinkIcon
